...
$arrSpecialMenuItems['newmembers'] = "<div style='margin-left: 0px; margin-right: 0px; margin-top: 5px; margin-bottom: 20px'>";
...
while($row = $result->fetch_assoc()) {
	...
	elseif($newestMemberInfo['profilepic'] == "") {
		$newestMemberInfo['profilepic'] = $MAIN_ROOT."themes/battlecity/images/defaultprofile.png";
	}
	
	
	$dispOnlineStatus = "<span style='color: pink'>Offline</span>";
	if($newestMemberInfo['loggedin'] == 1) {
		$dispOnlineStatus = "<span style='color: lime'>Online</span>";
	}
	...
	$arrSpecialMenuItems['newmembers'] .= "<div style='float: left; padding-left: 8px; width: 89px'><div style='font-size: 11px; font-weight: bold; overflow: hidden; text-overflow: ellipsis; width: 89px'>".$menuMemberObj->getMemberLink()."</div><div class='main' style='width: 89px; overflow: hidden; text-overflow: ellipsis'>".$menuMemberRankObj->get_info_filtered("name")."</div></div>";
	$arrSpecialMenuItems['newmembers'] .= "<div style='float: left; padding-left: 8px'>".$dispOnlineStatus."</div>";
...
$arrSpecialMenuItems['forumactivity'] = "<div style='margin-left: 0px; margin-right: 0px; margin-top: 5px; margin-bottom: 20px'>";
$counter = 0;
$postCount = 0;
...
while($row = $result->fetch_assoc()) {
	...
	if($blnShowPost) {
		...
		elseif($forumMemberInfo['avatar'] == "") {
			$forumMemberInfo['avatar'] = $MAIN_ROOT."themes/battlecity/images/defaultavatar.png";
		}
		...
		$arrSpecialMenuItems['forumactivity'] .= "<div class='main' style='float: left; width: 110px'><div style='font-size: 11px; overflow: hidden; text-overflow: ellipsis; width:115px'><b><a href='".$MAIN_ROOT."forum/viewtopic.php?tID=".$row['forumtopic_id']."#".$row['forumpost_id']."'>".$menuTopicPostInfo['title']."</a></b></div><div style='overflow: hidden; text-overflow: ellipsis; width:110px'>by ".$menuMemberObj->getMemberLink()."</div>".getPreciseTime($row['dateposted'])."</div>";
...
function dispMenu($intSectionNum) {
	...
	foreach($arrMenuCategories as $menuCatID) {
		...
		elseif($intSectionNum == 3) {
			echo "<img src='".$MAIN_ROOT."themes/battlecity/images/layout/top-menu-arrow.png' class='topMenu_Arrow' style=''>";	
		}
