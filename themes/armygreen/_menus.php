...
while($row = $result->fetch_assoc()) {
	...
	elseif($newestMemberInfo['profilepic'] == "") {
		$newestMemberInfo['profilepic'] = $MAIN_ROOT."themes/armygreen/images/defaultprofile.png";
	}
...
while($row = $result->fetch_assoc()) {
	...
	if($blnShowPost) {
		...
		elseif($forumMemberInfo['avatar'] == "") {
			$forumMemberInfo['avatar'] = $MAIN_ROOT."themes/armygreen/images/defaultavatar.png";
		}
			...
			switch($menuItemInfo['itemtype']) {
				...
				case "shoutbox":
					...
					if($menuShoutboxInfo['width'] == 0) {
						$menuShoutboxInfo['width'] = "85";
						$blnSBPercent = true;
					}
	
					$blnShoutboxWidthPercent = true;
					if($menuShoutboxInfo['percentwidth'] == 0 && !$blnSBPercent) {
