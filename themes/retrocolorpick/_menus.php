...
$arrSpecialMenuItems['newmembers'] = "<div style='margin-left: 3px; margin-right: 0px; margin-top: 5px; margin-bottom: 20px'>";
...
while($row = $result->fetch_assoc()) {
	...
	elseif($newestMemberInfo['profilepic'] == "") {
		$newestMemberInfo['profilepic'] = $MAIN_ROOT."themes/retrocolorpick/images/defaultprofile.png";
	}
...
$arrSpecialMenuItems['forumactivity'] = "<div style='margin-left: auto; margin-right: auto; margin-top: 5px; margin-bottom: 20px'>";
$counter = 0;
$postCount = 0;
...
while($row = $result->fetch_assoc()) {
	...
	if($blnShowPost) {
		...
		$arrSpecialMenuItems['forumactivity'] .= "<div class='dottedLine ".$addCSS."' style='padding: 5px 2px'>";
		$menuMemberObj->select($row['member_id']);
		...
		elseif($forumMemberInfo['avatar'] == "") {
			$forumMemberInfo['avatar'] = $MAIN_ROOT."themes/retrocolorpick/images/defaultavatar.png";
		}
		...
		foreach($arrMenuItems as $menuItemID) {
			...
			switch($menuItemInfo['itemtype']) {
				...
				case "image":
					...
					if($intSectionNum == 2) {
						echo "<div style='display: inline-block; text-align: ".$menuImageInfo['imagealign']."; margin: 0px 10px'>";
					}
					else {
				...
				case "shoutbox":
					...
					echo "<div class='navMenuLinks'>".$mainShoutboxObj->dispShoutbox($menuShoutboxInfo['width'], $menuShoutboxInfo['height'], $blnShoutboxWidthPercent, $menuShoutboxInfo['textboxwidth'], $blnShoutboxHeightPercent)."</div>";
