...
while($row = $result->fetch_assoc()) {
	...
	elseif($newestMemberInfo['profilepic'] == "") {
		$newestMemberInfo['profilepic'] = $MAIN_ROOT."themes/ghost/images/defaultprofile.png";
	}
...
while($row = $result->fetch_assoc()) {
	...
	if($blnShowPost) {
		...
		elseif($forumMemberInfo['avatar'] == "") {
			$forumMemberInfo['avatar'] = $MAIN_ROOT."themes/ghost/images/defaultavatar.png";
		}
		...
		foreach($arrMenuItems as $menuItemID) {
			...
			switch($menuItemInfo['itemtype']) {
				...
				case "image":
					...
					echo "<div style='text-align: ".$menuImageInfo['imagealign']."; margin: 15px 0px;'>";
