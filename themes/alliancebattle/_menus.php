...
$arrSpecialMenuItems['newmembers'] = "<div style='margin-left: 2px; margin-right: 2px; margin-top: 5px; padding-bottom: 10px'>";
...
while($row = $result->fetch_assoc()) {
	...
	if($blnShowPost) {
		...
		elseif($forumMemberInfo['avatar'] == "") {
			$forumMemberInfo['avatar'] = $MAIN_ROOT."themes/alliancebattle/images/defaultavatar.png";
		}
		
		$arrSpecialMenuItems['forumactivity'] .= "<div style=\"font-family: 'Fenix', verdana, sans-serif; float: left; width: 35px\"><img src='".$forumMemberInfo['avatar']."' style='width: 30px; height: 30px; border: solid black 1px'></div>";
		$arrSpecialMenuItems['forumactivity'] .= "<div class='main' style=\"font-family: 'Fenix', verdana, sans-serif; float: left; width: 100px\"><div style='font-size: 12px; overflow: hidden; text-overflow: ellipsis; width:100px'><a href='".$MAIN_ROOT."forum/viewtopic.php?tID=".$row['forumtopic_id']."#".$row['forumpost_id']."'>".$menuTopicPostInfo['title']."</a></div><div style='overflow: hidden; text-overflow: ellipsis; width:100px'>by ".$menuMemberObj->getMemberLink()."</div>".getPreciseTime($row['dateposted'])."</div>";
		...
		foreach($arrMenuItems as $menuItemID) {
			...
			switch($menuItemInfo['itemtype']) {
				...
				case "shoutbox":
					...
					echo $mainShoutboxObj->dispShoutbox($menuShoutboxInfo['width'], $menuShoutboxInfo['height'], $blnShoutboxWidthPercent, $menuShoutboxInfo['textboxwidth'], $blnShoutboxHeightPercent)."<br>";
