...
while($row = $result->fetch_assoc()) {
	...
	elseif($newestMemberInfo['profilepic'] == "") {
		$newestMemberInfo['profilepic'] = $MAIN_ROOT."themes/saintsrow/images/defaultprofile.png";
	}
	
	$menuMemberRankObj->select($newestMemberInfo['rank_id']);
	$arrSpecialMenuItems['newmembers'] .= "<div style='float: left; width: 42px'><img src='".$newestMemberInfo['profilepic']."' style='width: 40px; height: 53px; border: solid black 1px'></div>";
	$arrSpecialMenuItems['newmembers'] .= "<div style='float: left; padding-left: 5px; width: 82px'><div style='font-size: 11px; overflow: hidden; text-overflow: ellipsis; width: 82px'>".$menuMemberObj->getMemberLink()."</div><div class='main' style='font-size: 10px; width: 82px; overflow: hidden; text-overflow: ellipsis'>".$menuMemberRankObj->get_info_filtered("name")."</div></div>";
...
while($row = $result->fetch_assoc()) {
	...
	if($blnShowPost) {
		...
		elseif($forumMemberInfo['avatar'] == "") {
			$forumMemberInfo['avatar'] = $MAIN_ROOT."themes/saintsrow/images/defaultavatar.png";
		}
		...
		$arrSpecialMenuItems['forumactivity'] .= "<div class='main' style='float: left; width: 94px'><div style='font-size: 11px; overflow: hidden; text-overflow: ellipsis; width:94px'><a href='".$MAIN_ROOT."forum/viewtopic.php?tID=".$row['forumtopic_id']."#".$row['forumpost_id']."'>".$menuTopicPostInfo['title']."</a></div><div style='overflow: hidden; text-overflow: ellipsis; width:94px'>by ".$menuMemberObj->getMemberLink()."</div><span style='font-size: 10px'>".getPreciseTime($row['dateposted'])."</span></div>";
