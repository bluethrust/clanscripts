...
while($row = $result->fetch_assoc()) {
	...
	elseif($newestMemberInfo['profilepic'] == "") {
		$newestMemberInfo['profilepic'] = $MAIN_ROOT."themes/gtabeach/images/defaultprofile.png";
	}
...
while($row = $result->fetch_assoc()) {
	...
	if($blnShowPost) {
		...
		elseif($forumMemberInfo['avatar'] == "") {
			$forumMemberInfo['avatar'] = $MAIN_ROOT."themes/gtabeach/images/defaultavatar.png";
		}
...
function dispMenu($intSectionNum) {
	...
	foreach($arrMenuCategories as $menuCatID) {
		...
		if($intSectionNum == 0 && count($arrMenuItems) > 0) {
			echo "
