...
while($row = $result->fetch_assoc()) {
	...
	$arrSpecialMenuItems['newmembers'] .= "<div class='dottedLine ".$addCSS."' style='padding: 5px 2px'>";
	...
	elseif($newestMemberInfo['profilepic'] == "") {
		$newestMemberInfo['profilepic'] = $MAIN_ROOT."themes/bluefade/images/defaultprofile.png";
	}
	
	$menuMemberRankObj->select($newestMemberInfo['rank_id']);
	$arrSpecialMenuItems['newmembers'] .= "<div style='float: left; width: 34px'><img src='".$newestMemberInfo['profilepic']."' style='width: 30px; height: 40px; border: solid black 1px'></div>";
	$arrSpecialMenuItems['newmembers'] .= "<div style='float: left; padding-left: 8px; width: 80px'><div style='font-size: 11px; overflow: hidden; text-overflow: ellipsis; width: 80px; font-size: 11px'>".$menuMemberObj->getMemberLink()."</div><div class='main' style='width: 80px; overflow: hidden; text-overflow: ellipsis; font-size: 10px'>".$menuMemberRankObj->get_info_filtered("name")."</div></div>";
...
$arrSpecialMenuItems['forumactivity'] = "<div style='margin: 0px; margin-bottom: 10px'>";
$counter = 0;
$postCount = 0;
...
while($row = $result->fetch_assoc()) {
	...
	if($blnShowPost) {
		$arrSpecialMenuItems['forumactivity'] .= "<div class='dottedLine' style='padding: 5px 2px'>";
		$menuMemberObj->select($row['member_id']);
		...
		elseif($forumMemberInfo['avatar'] == "") {
			$forumMemberInfo['avatar'] = $MAIN_ROOT."themes/bluefade/images/defaultavatar.png";
		}
		...
		$arrSpecialMenuItems['forumactivity'] .= "<div class='main' style='float: left; width: 90px'><div style='font-size: 11px; overflow: hidden; text-overflow: ellipsis; width:90px'><a href='".$MAIN_ROOT."forum/viewtopic.php?tID=".$row['forumtopic_id']."#".$row['forumpost_id']."'>".$menuTopicPostInfo['title']."</a></div><div style='overflow: hidden; text-overflow: ellipsis; width:90px; font-size: 10px'>by ".$menuMemberObj->getMemberLink()."</div><span style='font-size: 10px'>".getPreciseTime($row['dateposted'])."</span></div>";
