...
if($member->authorizeLogin($_SESSION['btPassword']) && $member->hasAccess($consoleObj)) {
	...
	if($mysqli->query($sql)) {
		...
		$pluginID = array_search($_POST['pluginPath'], $pluginObj->getPlugins("filepath"));
		
		$pluginObj->select($pluginID);
		$checkDeletePlugin = $pluginObj->delete();
		
		// Remove Console Option
		$twitterConnectCID = $consoleObj->findConsoleIDByName("Twitter Connect");
		...
		if(!$checkDeletePlugin) {
			$countErrors++;
			$dispError[] = "Unable to delete plugin from database table.  You will have to manually delete it.";
		}
